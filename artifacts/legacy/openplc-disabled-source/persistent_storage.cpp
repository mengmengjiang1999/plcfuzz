...
void startPstorage() {
    ...
    while (pstorage_read == false) sleepms(100);
...
int readPersistentStorage() {
    ...
    for (int i = 0; i < BUFFER_SIZE; i++) {
        if (int_memory[i] != NULL)
            *int_memory[i] = persistentBuffer[i].int_cell;
        if (dint_memory[i] != NULL)
            *dint_memory[i] = persistentBuffer[i].dint_cell;
        if (lint_memory[i] != NULL)
            *lint_memory[i] = persistentBuffer[i].lint_cell;
    }
    pthread_mutex_unlock(&bufferLock);  // unlock mutex
