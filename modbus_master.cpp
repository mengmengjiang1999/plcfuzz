        ...
        for (int i = 0; i < num_devices; i++) {
            ...
            if (mb_devices[i].isConnected || rtu_port_connected) {
                ...
                if (mb_devices[i].coils.num_regs != 0) {
                    ...
                    nanosleep(&ts, NULL);
                    int return_val = modbus_write_bits(mb_devices[i].mb_ctx, mb_devices[i].coils.start_address,
                                                       mb_devices[i].coils.num_regs, tempBuff);
                    if (return_val == -1) {
                        if (mb_devices[i].protocol != MB_RTU) {
                            modbus_close(mb_devices[i].mb_ctx);
                            mb_devices[i].isConnected = false;
                ...
                if (mb_devices[i].input_registers.num_regs != 0) {
                    ...
                    nanosleep(&ts, NULL);
                    int return_val =
                        modbus_read_input_registers(mb_devices[i].mb_ctx, mb_devices[i].input_registers.start_address,
                ...
                if (mb_devices[i].holding_read_registers.num_regs != 0) {
                    ...
                    nanosleep(&ts, NULL);
                    int return_val =
                        modbus_read_registers(mb_devices[i].mb_ctx, mb_devices[i].holding_read_registers.start_address,
...
void initializeMB() {
    ...
    for (int i = 0; i < num_devices; i++) {
        if (mb_devices[i].protocol == MB_TCP) {
            ...
            if (share_index != -1) {
                if (mb_devices[i].rtu_baud != mb_devices[share_index].rtu_baud ||
                ...
                mb_devices[i].mb_ctx = modbus_new_rtu(mb_devices[i].dev_address, mb_devices[i].rtu_baud, mb_devices[i].rtu_parity,
                                                      mb_devices[i].rtu_data_bit, mb_devices[i].rtu_stop_bit);
