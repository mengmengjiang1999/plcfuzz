...
class CommandCallback : public ICommandHandler {
   public:
    ...
    virtual CommandStatus Operate(const AnalogOutputInt16& command, uint16_t index, OperateType opType) {
        ...
        } else if (index < MAX_16B_RANGE && int_memory[index - MIN_16B_RANGE] != NULL) {
    ...
    // AnalogOut 32 (Int)
    virtual CommandStatus Select(const AnalogOutputInt32& command, uint16_t index) { return CommandStatus::SUCCESS; }
    ...
    // AnalogOut 32 (Float)
    virtual CommandStatus Select(const AnalogOutputFloat32& command, uint16_t index) { return CommandStatus::SUCCESS; }
    ...
    // AnalogOut 64
    virtual CommandStatus Select(const AnalogOutputDouble64& command, uint16_t index) { return CommandStatus::SUCCESS; }
...
void update_vals(std::shared_ptr<IOutstation> outstation) {
    ...
    for (int i = MIN_16B_RANGE; i < MAX_16B_RANGE; i++) {
        if (int_memory[i - MIN_16B_RANGE] != NULL)
            builder.Update(AnalogOutputStatus((int)(*int_memory[i - MIN_16B_RANGE])), i);
    }
    // Update Holding registers for 32 b memory
    for (int i = MIN_32B_RANGE; i < MAX_32B_RANGE; i++) {
        if (dint_memory[i - MIN_32B_RANGE] != NULL)
            builder.Update(AnalogOutputStatus((int)(*dint_memory[i - MIN_32B_RANGE])), i);
    }
    // Update Holding registers for 64 b memory
    for (int i = MIN_64B_RANGE; (i < MAX_64B_RANGE && i - MIN_64B_RANGE < sizeof(lint_memory) / sizeof(lint_memory[0])); i++) {
        if (lint_memory[i - MIN_64B_RANGE] != NULL)
            builder.Update(AnalogOutputStatus((int)(*lint_memory[i - MIN_64B_RANGE])), i);
    }
        ...
        while (getline(cfgfile, line)) {
            ...
            try {
                ...
                if (token == "database_size") {
                    getline(iss, token, '=');
                    return OutstationStackConfig(DatabaseSizes::AllTypes(atoi(token.c_str())));
                } else
                    continue;
            } catch (...) {
            ...
            try {
                ...
                if (token == "local_address") {
                    ...
                    if (token == "MAX") {
                        ...
                        config.link.KeepAliveTimeout = openpal::TimeDuration::Seconds(atoi(token.c_str()));
                    }
                    ...
                    getline(iss, token, '=');
                    config.outstation.params.selectTimeout = openpal::TimeDuration::Seconds(atoi(token.c_str()));
                } else if (token == "max_controls_per_request") {
                    getline(iss, token, '=');
                    config.outstation.params.maxControlsPerRequest = atoi(token.c_str());
                } else if (token == "max_rx_frag_size") {
                    getline(iss, token, '=');
                    config.outstation.params.maxRxFragSize = atoi(token.c_str());
                } else if (token == "max_tx_frag_size") {
                    getline(iss, token, '=');
                    config.outstation.params.maxTxFragSize = atoi(token.c_str());
                } else if (token == "event_buffer_size") {
                    getline(iss, token, '=');
                    config.outstation.eventBufferConfig = EventBufferConfig::AllTypes(atoi(token.c_str()));
                    ...
                    getline(iss, token, '=');
                    config.outstation.params.solConfirmTimeout = openpal::TimeDuration::Milliseconds(atoi(token.c_str()));
                } else if (token == "unsol_confirm_timeout") {
                    getline(iss, token, '=');
                    config.outstation.params.unsolConfirmTimeout = openpal::TimeDuration::Milliseconds(atoi(token.c_str()));
                } else if (token == "unsol_retry_timeout") {
                    getline(iss, token, '=');
                    config.outstation.params.unsolRetryTimeout = openpal::TimeDuration::Milliseconds(atoi(token.c_str()));
                }
            } catch (...) {
                cout << "Malformatted Line: " << line << endl;
                exit(1);
...
void dnp3StartServer(int port) {
    ...
    auto channel =
        manager.AddTCPServer("DNP3_Server", FILTERS, ChannelRetry::Default(), "0.0.0.0", port, PrintingChannelListener::Create());

    // Create a new outstation with a log level, command handler, and
    // config info this returns a thread-safe interface used for
    // updating the outstation's database.
    std::shared_ptr<ICommandHandler> cc = std::make_shared<CommandCallback>();
    auto outstation = channel->AddOutstation("outstation", cc, DefaultOutstationApplication::Create(), parseDNP3Config());
