...
//         // PCCC type 1 - Unknown
//         return 1;
//     } else if (enipDataUnknown->item1_data[0] == 0xb2 &&
//                enipDataUnknown->item2_length[1] == 0x4b)  // There is an offset of the bytes within the
//     {                                                     // Unconnected and Connected type data
//         // PCCC type 2 - Unconnected Data Item														   // that is accounted for to
//         // check enipType														   // This means the labels "item1_data and
//         // item2_length
//         return 2;  // do not contain what is stated but what it would be for the correct type
//     } else if (enipDataUnknown->item1_data[0] == 0xb2 && ((enipDataUnknown->item2_length[1] == 0x54) ||
//                                                           (enipDataUnknown->item2_length[1] == 0x4e)))  // 0x54 opens connection
...
//     uint16_t dataLength =
//         ((uint16_t)enipDataConnected_0x70->item2_length[1] << 8) | (uint16_t)enipDataConnected_0x70->item2_length[0];
//     return dataLength;
// }
...
// // Command Code: 0x65
// //-----------------------------------------------------------------------------
// int sendRRData(int enipType, struct enip_header *header, struct enip_data_Unknown *enipDataUnknown,
//                struct enip_data_Unconnected *enipDataUnconnected, struct enip_data_Connected *enipDataConnected) {
//     if (enipType == 1) {
...
//     uint16_t newHeaderSize = newPcccSize + newItem2Size + 14 + masked_write_check + write_check +
//                              write_check_2;  // Use to be '+ 16' uint16_t newHeaderSize = newPcccSize + newItem2Size + 16;
