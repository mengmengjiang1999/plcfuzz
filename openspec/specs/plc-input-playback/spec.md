...
The runtime SHALL obtain one complete simulated PLC input snapshot per hardware input update and SHALL use that same snapshot for every typed input assignment in the update.
...
- **THEN** every typed stream advances exactly once and all assignments use the returned composite snapshot
