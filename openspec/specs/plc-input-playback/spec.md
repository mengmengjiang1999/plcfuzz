...
The runtime SHALL obtain one complete simulated PLC input snapshot per hardware input update, SHALL use that same snapshot for every typed input assignment in the update, and SHALL apply it only to automated-input slots within the complete runtime buffer.
...
- **WHEN** the runtime buffer has more address slots than the automated-input model
